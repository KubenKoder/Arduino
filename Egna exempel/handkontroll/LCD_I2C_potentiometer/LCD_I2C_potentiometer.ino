...
void loop()
{
 // read the input on analog pin 0:
  int sensorVerdi = analogRead(A0);
    Serial.println(sensorVerdi);   // print out the value you read to the PC for checking that it works
  //Convert it to a value between 0 and 100
  int percent = map(sensorVerdi,0,1023,0,100); //map(value, fromLow, fromHigh, toLow, toHigh) 
  //Print the value
